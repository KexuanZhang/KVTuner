...
parser.add_argument('--quant_scheme', type=str, required=False, default='pertoken')
...
def get_filename(model, quant_scheme, task_filename, nshots, kvconfig_or_bf16_or_id):
...
with open(out_filename, 'w+') as f:
    ...
    for model in models:
        filename_model = model.replace('/', '_') + f'_{quant_scheme}_baseline_limit200'
        ...
        for kv_config in STANDARD_KV_CONFIG:
            for task_preset in TASKS:
                for nshot in task_preset['nshots']:
                    ...
                    filename = get_filename(filename_model, quant_scheme, task_preset['filename'], nshot, kv_config)
