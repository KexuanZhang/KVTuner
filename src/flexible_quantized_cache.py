...
class FlexibleQuantizedCache(DynamicCache):
    ...
    def update(
        ...
        if len(self.key_cache) < layer_idx:
            ...
            self._quantized_key_cache.append(self._quantize(key_states.contiguous(), axis=self.axis_key, nbit=self.nbits_key))
            self._quantized_value_cache.append(self._quantize(value_states.contiguous(), axis=self.axis_value, nbit=self.nbits_value))
            ...
            if (
                ...
                self._quantized_key_cache[layer_idx] = self._quantize(keys_to_return.contiguous(), axis=self.axis_key, nbit=self.nbits_key)
                self._quantized_value_cache[layer_idx] = self._quantize(
                    values_to_return.contiguous(), axis=self.axis_value, nbit=self.nbits_value
                )
...
class FlexibleQuantoQuantizedCache(FlexibleQuantizedCache):
    ...
    def _quantize(self, tensor, axis, nbit):
        # We have two different API since in optimum-quanto, we don't use AffineQuantizer anymore
        if is_optimum_quanto_available():
            from optimum.quanto import quantize_weight

            qtensor = quantize_weight(tensor, self.get_qtype(nbit), axis, self.q_group_size)
            return qtensor
        elif is_quanto_available():
            logger.warning_once(
                "Importing from quanto will be deprecated in v4.47. Please install optimum-quanto instead `pip install optimum-quanto`"
            ...
            scale, zeropoint = self.optimizer(tensor, nbit, axis, self.q_group_size)
            qtensor = AffineQuantizer.apply(tensor, self.get_qtype(nbit), axis, self.q_group_size, scale, zeropoint)
